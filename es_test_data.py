...
def delete_index(idx_name):
    try:
        url = f"{tornado.options.options.es_url} {idx_name}"
        ...
        logging.error("Delete index error: %s %s", e, e.response.body, exc_info=e)
...
def create_index(idx_name):
    ...
    try:
        ...
        logging.error("Create index error: %s %s", e, e.response.body, exc_info=e)
...
def upload_batch(upload_data_txt):
    ...
    logging.info(
        ...
        % (res_txt, took, upload_data_count)
    )
...
def get_data_for_format(format):
    ...
    if field_type == "arr":
        ...
        single_elem_format = (
            field_name
            + ":"
            + format[len(field_name) + len(field_type) + len(array_len_expr) + 3 :]
        )
        ...
        return_val = (
            int(ts * 1000)
            if field_type == "ts"
            else datetime.datetime.fromtimestamp(ts).strftime(
        ...
        text = (
            ["text1", "text2", "text3"] if len(split_f) < 3 else split_f[2].split("-")
        )
...
def generate_test_data():
    ...
    if tornado.options.options.dict_file:
        ...
        logging.info(
            ...
            % (len(_dict_data), tornado.options.options.dict_file)
        )
    ...
    if tornado.options.options.data_file:
        ...
        if tornado.options.options.data_file.endswith(".csv"):
            ...
            logging.info(
                "Loaded documents from the %s", tornado.options.options.data_file
            )
        ...
        logging.info(
            ...
            % (tornado.options.options.count, tornado.options.options.batch_size)
        )
...
if __name__ == "__main__":
    ...
    tornado.options.define(
        "count", type=int, default=100000, help="Number of docs to generate"
    )
    tornado.options.define(
        "format",
        type=str,
    ...
    tornado.options.define(
        "num_of_replicas", type=int, default=0, help="Number of replicas for ES index"
    )
    tornado.options.define(
        "force_init_index",
        type=bool,
    ...
    tornado.options.define(
        "data_file", type=str, default=None, help="Name of the documents file to use"
    )
    tornado.options.define(
        "username", type=str, default=None, help="Username for elasticsearch"
    )
    tornado.options.define(
        "password", type=str, default=None, help="Password for elasticsearch"
    )
    tornado.options.define(
        "validate_cert",
        type=bool,
